...
# Making a slim dataset for all items
d_slim <- d_w %>%
  filter(!grepl("sum", item)) %>%
  mutate(item = gsub(".daily", "", item),
         item = gsub("avg.", "", item)) %>%
  mutate(
    ...
    weekday_num = lubridate::wday(date,
                                  week_start = 1,
                                  label = FALSE)
  ) %>% select(id, t, y, item, weekday, weekday_num)

items <- d_slim$item %>% unique()
...
fit_logs <- foreach(i = 1:nrow(conditions_table)) %dopar% {
  ...
  # Getting the data
  d_slim_i <- d_slim %>% filter(id == t_c$id,
                                item == t_c$item)
  ...
  dump <- tryCatch({
    paste0(t_c$mu,
      ...
      m_fit(
        ...
        .,
        id = t_c$id,
        save_prefix_fit = paste0(t_c$item,
                                 "_fit"),
        save_prefix_est = paste0(t_c$item,
                                 "_est"),
        save_fit = TRUE,
        save_est = TRUE
      )
...
# Saving the large raw harvested dataframe of all results
saveRDS(harvest, "harvest_all-items.rds")

# Reading the large raw harvested dataframe of all results
readRDS(harvest, "harvest_all-items.rds")
...
results_estimates <- harvest %>%
  ...
  mutate(
    ...
    error_timescale = case_when(
      ...
      factor(levels = c("wn", "daily", "weekly", "daily + weekly")),
...
saveRDS(results_estimates, "results_estimates_all-items.rds")

# Reporting the results ---------------------------------------------------

# Reading the processed estimates & ICs results dataframe
results_estimates <- readRDS("results_estimates_all-items.rds")

# Counting selected models]
r_win <- results_estimates %>%
...
# Saving the winners of model comparison results extracted results_estimates
saveRDS(r_win, "r_win_all-items.rds")

# Reporting the results ---------------------------------------------------

# Reading the winners of model comparison results extracted results_estimates
r_win <- readRDS("r_win_all-items.rds")

## Making the tables
## For other ICs, change AICc to AIC or BIC

# Making the detailed table with 16 rows
xtabs(n_w_AICc  ~ item + error_name_words + mu, r_win)

# Main body of the summarized table
xtabs(n_w_AICc ~ error_dynamics + mu + error_timescale + item,
      r_win)
# Rows total per mu (Total a)
xtabs(n_w_AICc ~ error_dynamics + mu + item,
      r_win)
# Rows sum all mu (Sum b)
xtabs(n_w_AICc ~ item + error_dynamics,
      r_win)
# Columns total per mu (Total c)
xtabs(n_w_AICc ~  error_timescale + mu  + item,
      r_win)
# Column sum all mu (Sum d)
xtabs(n_w_AICc ~ item + error_timescale,
      r_win)
# Totals per mu
