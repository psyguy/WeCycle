...
m_fit <- function(df,
  ...
  mu <- "c"
  if(grepl("d", model_string, fixed = TRUE)) mu <- "d"
  if(grepl("h", model_string, fixed = TRUE)) mu <- "h"
...
calc_amp <- function(a, b) sqrt(a^2 + b^2)
# Vectorizing it
...
m_estimates <- function(m,
  ...
  estimates <- data.frame(parameter = "sigma2",
                          est = m$sigma2,
                          se = NA,
                          CI_2.5 = NA,
                          CI_97.5 = NA,
  ...
  if(length(o_est) > 0)
    estimates <- data.frame(parameter = o_est %>% names,
                            est = o_est,
                            se = o_sd) %>%
    mutate(CI_2.5 = est + qnorm(0.025)*se,
  ...
  if(sum(names(m$coef) == "a_cos",
    ...
    o_vcov_h <- o_vcov[c("c", "a_cos", "b_sin"),c("c", "a_cos", "b_sin")]
    ...
    b_psi_CIs <- as.numeric(quantile(boot_theta, c(0.025, 0.975)))*7/(2*pi)

    # Adding the estimates to the table
    boot_estimates <- data.frame(parameter = c("amp", "peak_shift"),
                                 est = c(b_s, b_psi),
  ...
  estimates[2:4] <- estimates[2:4] %>% round(4)
